...
from dummy_data import dummy_pi_bytes
...
if 'pi_is_dummy' not in st.session_state:
    st.session_state.pi_is_dummy = False


# ─────────────────────────────────────────────────────────────────────────────
# AUTO-LOAD DUMMY DATA on first visit (so dashboard isn't empty)
# ─────────────────────────────────────────────────────────────────────────────
if st.session_state.pi_analysis is None:
    try:
        _dummy_bytes = dummy_pi_bytes(seed=42)
        st.session_state.pi_analysis = cached_pi_compute(_dummy_bytes, "_dummy_data.xlsx")
        st.session_state.pi_file_bytes = _dummy_bytes
        st.session_state.pi_uploaded_file_name = "_dummy_data.xlsx"
        st.session_state.pi_uploaded_file_size = len(_dummy_bytes)
        st.session_state.pi_is_dummy = True
    except Exception:
        pass
...
if file_changed:
    ...
    if st.button("🚀 Process Data", type="primary", key='pi_process'):
        with st.spinner("Running PI analysis (fast path)..."):
            ...
            st.session_state.pi_is_dummy = False  # real data now
            st.session_state.pi_excel_ready = False  # rebuild excel for new data
            if 'pi_excel_bytes' in st.session_state:
                del st.session_state.pi_excel_bytes
