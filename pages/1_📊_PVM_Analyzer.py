...
from dummy_data import dummy_pvm_bytes
...
if 'pvm_is_dummy' not in st.session_state:
    st.session_state.pvm_is_dummy = False

# ─────────────────────────────────────────────────────────────────────────────
# AUTO-LOAD DUMMY DATA on first visit (so dashboard isn't empty)
# ─────────────────────────────────────────────────────────────────────────────
if st.session_state.pvm_analysis is None:
    try:
        _dummy_bytes = dummy_pvm_bytes(seed=42)
        st.session_state.pvm_analysis = cached_pvm_compute(_dummy_bytes, "_dummy_data.xlsx")
        st.session_state.pvm_file_bytes = _dummy_bytes
        st.session_state.pvm_uploaded_file_name = "_dummy_data.xlsx"
        st.session_state.pvm_uploaded_file_size = len(_dummy_bytes)
        st.session_state.pvm_is_dummy = True
    except Exception:
        pass
...
# Banner if dummy data is loaded
    ...
    if st.button("🚀 Process Data", type="primary", use_container_width=False):
        with st.spinner("Processing... (fast path ~1 detik)"):
            try:
                ...
                st.session_state.pvm_is_dummy = False  # real data now
                st.session_state.pvm_excel_ready = False  # rebuild excel for new data
                if 'pvm_excel_bytes' in st.session_state:
                    del st.session_state.pvm_excel_bytes
